...
{
    public class LockthreatJwtSecurityTokenHandler : ISecurityTokenValidator
    {
        private readonly JwtSecurityTokenHandler _tokenHandler;
        public LockthreatJwtSecurityTokenHandler()
        {
        ...
        private static void ValidateUserDelegation(ClaimsPrincipal principal)
        {
            ...
            var tenant = principal.Claims.FirstOrDefault(c => c.Type == AbpClaimTypes.TenantId);
            ...
            {
                if (_permissionChecker.Object.IsGranted(new UserIdentifier(tenantId, targetUserId), AppPermissions.Pages_Administration_Users_Impersonation)){
                    return;
                }
            }
            ...
            {
                var hasActiveDelegation = userDelegationManager.Object.HasActiveDelegation(sourceUserId, targetUserId);
